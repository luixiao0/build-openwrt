...
- #### 2021年2月19号改版最新编译脚本，此次脚本增加一个自定义文件，这个文件是每次启动编译时候拉取的，这个文件是由我修改的，这样是为了有时候出现一些小错误而我又可以解决的，又能在这个自定义文件内可以用命令解决的，我就可以在里面增加一个命令就搞定了，那就不需要你们自己动手搞，这样大大的方便了大家使用脚本，当然，前提是有错误的时候有人提出来，我也有时间弄才行，但总比没有的好，还有就是把定时更新插件也弄进去了，编译的时候要不要编译上就看你们自己了，不是什么机子都支持定时更新的（此次更新增加了所有的常用插件）
